...
object CountWindowDemo {
  ...
  def test02(env: StreamExecutionEnvironment) = {
     ...
     */
    val windowStream: WindowedStream[SensorReading, String, GlobalWindow] = inputStream
      .keyBy(x => x.id)
      .countWindow(5)//收到5次消息统计一次
...
reduce==> SensorReading(sensor_1,8000,5.0)
   */

  /**
   * 第一次计算输出总数：5,数据sensor_1,8000,1.0没有计算在内
   * 已经够5条记录
   */
  /**
   * SensorReading(sensor_1,1000,1.0)
...
SensorReading(sensor_1,28000,1.0)
SensorReading(sensor_1,38000,1.0)
SensorReading(sensor_1,39000,1.0)
   */
  //没有触发
  ...
  /**
   * 输出结果3：
   * reduce==> SensorReading(sensor_1,49000,5.0)
SensorReading(sensor_1,49000,1.0)
   */
